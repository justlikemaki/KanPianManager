...
public class BaseAction extends Controller {
	 ...
	 * @版权: Copyright (c) 2014
	 * @公司: VETECH
	 * @作者：LF @时间：2014-8-9 @版本：1.0 @方法描述： 删除数据，返回成功状态。
	 */
	 ...
	 * @版权: Copyright (c) 2014
	 * @公司: VETECH
	 * @作者：LF @时间：2014-8-9 @版本：1.0 @方法描述： 上传图片和种子
	 */
	 ...
	 * @版权: Copyright (c) 2014
	 * @公司: VETECH
	 * @作者：LF @时间：2014-8-9 @版本：1.0 @方法描述： 上传exl数据，解析并保存到资源库，记录数据到表中。
	 */
				...
				if (!goflag) {
					...
					for (int j = 0; j < ine.getFilePath().length; j++) {
						...
						if (filenames.indexOf(".torrent") >= 0) {
							...
							filepath = PageKit.copypath(request, ine.getTypeDirMc(), ine.getParentDirMc(),
									filenames.replace(filenm, ine.getParentDirMc()));
						}
	...
	private String dosomthing(String url, int threadnum, int trynum, String filename, HttpServletRequest request) {
		try {
			...
			if (!ctype.contains("text/html")) {
				...
				if (p.get(name) == null) {
					TaskThreadManagers ttm = MultitDownload.instanceMultitDownloadSetAll(contentLength, url, filename,
							threadnum, trynum);
					p.put(name, ttm);
					request.getServletContext().setAttribute("download", p);
					return "添加下载成功";
