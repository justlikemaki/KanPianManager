...
public class PgsqlKit {
			...
			while (it.hasNext()) {
				...
				if (pairs.getValue() != null && StringUtils.isNotBlank(pairs.getValue() + "")) {
					...
					if (operator.contains(opkey)) {
						...
						if (pairs.getKey().equals("id") || pairs.getKey().equals("tabtype")
								|| pairs.getKey().equals("type")) {
	...
	// public static void savebk(String collectionName,Object obj) throws
	// Exception{
	...
	// String sql = "select * from javsrc order by id asc limit "+count+" offset
	// "+offset;
	// try {
	...
	public static Map excuteSqlPaginate(String sql, Class<?> clazz, SearchQueryP p) throws Exception {
		Map res = new HashMap();
		Page<Record> pr = Db.paginate(p.getNowpage(), p.getCount(), "select * ", sql);
		res.put("list", BeanKit.copyRec(pr.getList(), clazz));
		res.put("count", pr.getTotalRow());
